...
export class LEDService {
    ...
    public addCron(cronjob: CronJob) {
        if (cronjob && cronjob.pattern && cronjob.pattern.trim() !== "" && cronjob.cron && cronjob.cron.trim() !== "") {
            this.getCom().write(this.BASE + "cron/add", {cron: cronjob}).catch(this.logError("adding cronjob"));
        } else {
          this.logError("Please fill all values");
        }
    }

    public removeCron(cronjob: CronJob) {
        this.getCom().write(this.BASE + "cron/remove", {cron: cronjob}).catch(this.logError("removing cronjob"));
    }

    public getCrons(): Promise<CronJob[]> {
        return this.getCom().read(this.BASE + "cron/get").then(result => result.cronJobs, this.logError("getting cronjob"));
    }
}
